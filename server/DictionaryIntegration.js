...
function renderDictionaryContent(contentString) {
  ...
  tokens.forEach(token => {
    if (token.startsWith('[AUDIO:')) {
      let url = token.substring(7, token.length - 1); // Extract URL
      // Optional: Add ttps:// check for audio if you suspect it too
      // if (url.startsWith('ttps://')) {
      //   Logger.log(`WARNING: Detected 'ttps://' in audio URL: ${url}. Correcting to 'https://'.`);
      //   url = 'h' + url;
      // }
      const safeUrl = escapeHtmlServerSide(url); // Uses corrected function
      resultHtml += `
      ...
      let url = token.substring(8, token.length - 1); // Extract URL
      
      // This block is for the ttps:// issue
      if (url.startsWith('ttps://')) {
        Logger.log(`WARNING: Detected 'ttps://' in image URL in renderDictionaryContent: ${url}. Attempting correction.`);
        url = 'h' + url; // Correcting ttps:// to https://
        Logger.log(`Corrected image URL to: ${url}`);
      } else if (url.startsWith('ttp://')) {
        Logger.log(`WARNING: Detected 'ttp://' in image URL: ${url}. Consider if it should be 'https://'.`);
      }
      ...
      resultHtml += `
        <div class="flashcard-image">
          <img src="${safeUrl}" alt="Illustration" class="centered-card-image" onerror="this.alt='Image failed to load'; this.parentElement.innerHTML='<p style=\\'color:red; font-size:small;\\'>Image failed to load.<br>URL: ${escapeHtmlServerSide(url.length > 100 ? url.substring(0,100)+'...' : url)}</p>'; console.error('Failed to load image:', this.src);">
        </div>
      `;
    } else if (token.startsWith('[Source:')) {
      // const sourceText = token.substring(8, token.length - 1); // Extract source text
      // Source text is hidden by CSS on the flashcard.
